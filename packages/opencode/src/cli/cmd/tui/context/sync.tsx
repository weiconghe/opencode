          ...
          const task = (async () => {
            ...
            setStore(
              produce((draft) => {
                ...
                const fetched = (messages.data ?? []).map((item) => {
                  if (!tracker.messages.has(item.info.id)) return item
                  const current = currentMessages.find((message) => message.id === item.info.id)
                ...
                const fetchedWithHydratedParts = fetched.map((item) => {
                  const current = draft.part[item.info.id] ?? []
                  const parts = item.parts.flatMap((part) => {
                    const currentPart = current.find((value) => value.id === part.id)
                    if (tracker.parts.has(part.id)) return currentPart ? [currentPart] : []
                    if (
                      currentPart &&
                      (part.type === "text" || part.type === "reasoning") &&
                      (currentPart.type === "text" || currentPart.type === "reasoning") &&
                      part.text.length === 0 &&
                      currentPart.text.length > 0
                    ) {
                      return [currentPart]
                    }
                    return [part]
                  })
                  parts.push(...current.filter((part) => tracker.parts.has(part.id) && !parts.some((x) => x.id === part.id)))
                  return {
                    info: item.info,
                    parts,
                  }
                })
                const preserveMessages = new Set(optimisticMessages)
                for (const messageID of tracker.messages) preserveMessages.add(messageID)
                const merged = mergeFetchedMessages({
                  currentMessages,
                  currentParts: draft.part,
                  fetched: fetchedWithHydratedParts,
                  optimisticMessages: preserveMessages,
                })
